...
def test_linear_batchnorm_relu_forward():
    np.random.seed(11785)
    mytorch_mlp = Sequential(Linear(10, 20), BatchNorm1d(20), ReLU(), Linear(20, 20))
    ...
    for l in mytorch_model.layers:
        if isinstance(l, Linear):
            ...
            layers[-1].weight = nn.Parameter(
                torch.tensor(l.weight.data, dtype=torch.float64).double())
            layers[-1].bias = nn.Parameter(torch.tensor(l.bias.data, dtype=torch.float64).double())
        elif isinstance(l, BatchNorm1d):
            layers.append(nn.BatchNorm1d(int(l.num_features)))
            layers[-1].weight = nn.Parameter(
                torch.tensor(l.gamma.data).double(), dtype=torch.float64)
            layers[-1].bias = nn.Parameter(torch.tensor(l.beta.data, dtype=torch.float64).double())
...
def forward_(mytorch_model, mytorch_criterion, pytorch_model,
    ...
    """
    # forward
    pytorch_x = Variable(torch.tensor(x, dtype=torch.float64).double(), requires_grad=True)
    ...
    # print(mytorch_y.data, pytorch_y.detach().numpy())
    # print('-----------------------------------------')
    # forward check
