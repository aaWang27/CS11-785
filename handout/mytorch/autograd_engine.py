...
def backward(grad_fn, grad_of_outputs):
    ...
    temp_grad = grad_fn.apply(grad_of_outputs)
    # 2) Pass gradient onto current node's beloved parents (recursive DFS)
    idx = 0

    for parent in grad_fn.next_functions:
        if not(parent==None):
            if type(temp_grad).__name__ == 'tuple':
                backward(parent, temp_grad[idx])
            elif type(temp_grad).__name__ == 'Tensor':
                backward(parent, temp_grad)
        idx += 1
                ...
                if parent.requires_grad:
                    if parent.is_leaf:
                        if parent.grad_fn == None:
                            backward_function.next_functions.append(AccumulateGrad(parent))
                        else:
                            backward_function.next_functions.append(parent.grad_fn)
                    else:
                        if parent.grad_fn == None:
                            backward_function.next_functions.append(BackwardFunction(cls))
                        else:
                            backward_function.next_functions.append(parent.grad_fn)
                else:
                    if parent.is_leaf:
